...
int main(int argc, char **argv) {
    if(argc < 8) {
        ...
        logger(INFO_LOG, "example: %s \"-pi/1.5\" \"pi/8\" 0.1 \"-pi/2\" \"pi/2\" 0.05 \"y*y+x*x+y-SQRT(y*y+x*x)\" 2>errs.log 1>out", argv[0]);
        logger(INFO_LOG, "example: %s \"-pi\" \"1\" 0.1 \"-2\" \"2\" 0.05 \"(ACOS(1-FABS(x))-pi)-y\" \"y-SQRT(1-(FABS(x)-1)^2)\" 2>errs.log 1>out", argv[0]);
        logger(INFO_LOG, "example: %s \"-1\" \"pi/2\" 0.1 \"-1\" \"1\" 0.05 \"x*x+(y-FABS(x)^(2.0/3.0))^2-1\" 2>errs.log 1>out", argv[0]);
        exit(0);
    }
    INIT(argv + 1);
